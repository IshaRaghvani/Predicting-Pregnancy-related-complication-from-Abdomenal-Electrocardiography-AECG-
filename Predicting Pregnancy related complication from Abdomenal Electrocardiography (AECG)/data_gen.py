...
#'''Cluster extraction based on labels
...
# Create data_for_cluster instances for each label
...
for i in range(1800 - len(normal)):
	...
	if i%50==0:
		print(i,"rows filled for normal class.", row)
	while True:
...
for i in range(1800 - len(suspect)):
	...
	if i%50==0:
		print(i,"rows filled for suspect class.", row)
	while True:
...
for i in range(1800 - len(pathologic)):
	...
	row += 1
	if i%50==0:
		print(i,"rows filled for pathological class.", row)
	while True:
